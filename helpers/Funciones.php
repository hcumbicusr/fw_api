...
//------------PHPmailer
  ...
  public static function genetareAccessToken() {
  ...
  public static function validateToken() {
    global $adb;
    if ( empty($_SERVER['PHP_AUTH_USER']) || empty($_SERVER['PHP_AUTH_PW'])) return false;
    $user = $_SERVER['PHP_AUTH_USER'];
    $token = $_SERVER['PHP_AUTH_PW'];
    $val = $adb->pquery("SELECT * FROM `user_token` u WHERE (u.token = '$token') and u.active = 1 AND (u.fin < NOW() OR u.fin IS NULL)");
    if (!empty($val)) {
      $adb->pquery("UPDATE user_token set count_request = IFNULL(count_request,0)+1 WHERE token = '$token' ");
      return true;
    }
    return false;
  }
  ...
  /**
  * @return nro de día de la semana o el nombre del día 
  */
    public static function getDiaSemana($fecha_hora, $retur_nro = true) {
      $dias = array('Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo');
      $dia = date('N', strtotime($fecha_hora)); // 1= Lunes
      if (!$retur_nro) $dia = $dias[$dia-1];
      return $dia;
    }

    public static function validaAsistencia($trabajador, $fecha_hora, $nro_dia, $in_out='IN', $ultima_asistencia) {
      global $adb;
      $Configuracion = $adb->pquery("SELECT * FROM configuraciones LIMIT 1");
      $Configuracion = (object) $Configuracion[0];      
      // $Configuracion = Configuracion::first();
      $tolerancia = !empty($Configuracion)? intval($Configuracion->tolerancia_min) : 0;
      $repetir_scan_min = !empty($Configuracion)? intval($Configuracion->repetir_scan_min) : 5;
      $is_horario_estricto = !empty($Configuracion)? intval($Configuracion->is_horario_estricto) : 0;

      $a_fecha_hora = $a_nro_dia = $a_in_out = null;
      if ( !empty($ultima_asistencia) ) {
          $a_fecha_hora = $ultima_asistencia->fecha_hora;
          $a_nro_dia = $ultima_asistencia->nro_dia;
          $a_in_out = $ultima_asistencia->in_out;
          $uf_reg = new \DateTime($a_fecha_hora);
          $reg_hoy = new \DateTime($fecha_hora);

          $intervalo = $uf_reg->diff($reg_hoy);
          $minutos = ($intervalo->h * 60) + $intervalo->i;
            // return $repetir_scan_min;
            // if ( intval($uf_reg->format("d")) < intval($reg_hoy->format("d")) ) {
          if ( $a_in_out == $in_out ) {
            if ( $repetir_scan_min > $minutos ) {
              $formato = $uf_reg->format("d-m-Y h:i:s A");
                $mensaje_inout = $in_out=='IN'? 'un INGRESO' : 'una SALIDA';
                return [0, "[DUPLICADO] Ya se ha registrado $mensaje_inout: $formato"];
            } else {
                return [1, "[OK]"];
            }
            }
