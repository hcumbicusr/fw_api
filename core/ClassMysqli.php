...
class ClassMysqli implements IDatabase{
        ...
        try {
            if (empty($database)) {
                ...
                $this->con = new mysqli(
                        ...
                        $config['accessBD_'.$database]['port']);
            }
    ...
    public function pquery($query, $close = false){
        global $adb;
        if($this->isTest) {
            $this->close();
            die(var_dump($query)); 
        ...
        if ( strpos(strtolower($query), "select") === false ) return $consult;
        $data = [];
        if ($consult != NULL)
        {
